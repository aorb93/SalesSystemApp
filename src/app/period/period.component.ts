...
import { DialogPeriodComponent } from './dialog-period/dialog-period.component';
...
public faEdit = faEdit;
  ...
  constructor( private apiPeriod: ApiPeriodService, public dialog: MatDialog, public snackBar: MatSnackBar, private spinner: NgxSpinnerService, private isMobile: AppComponent) {
  ...
  openAdd(){
    ...
    const dialogRef = this.dialog.open(DialogPeriodComponent, {
      width: this.width,
      height: this.height,
      autoFocus: false
    });
  ...
  delete(period: Period){
    ...
    dialogRef.afterClosed().subscribe(result => {
      if(result){
        this.apiPeriod.delPeriod(tmpPeriod).subscribe(response => {
          if(response){
            this.snackBar.open('Talla ' + period.periodName + ' eliminada con exito', '', {
              duration: 3000,
